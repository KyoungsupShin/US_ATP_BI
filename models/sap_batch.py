...
class SAP_Master_Reset(Email_Utils):    
    ...
    def atp_raw_history_batch(self):
        ...
        view_names = [
                        ['ATP_ONHAND_SAP', 'ON_HAND_SAP_HISTORY', ['Product_Name', 'WH_3PL_Name']] 
    ...
    def data_null_check(self, table_name, join_check_column_list):
        ...
        if len(nullcheck_total_result) > 0:
            ...
            self.send_email('[RPA WARNING] {} JOIN CHECKING NOT VERIFIED'.format(table_name)
                            ,'ERROR MESSAGE'
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            if int(datetime.datetime.now().hour) == 10 and int(datetime.datetime.now().minute) == 45:
                smr.read_qspdb()
                smr.update_sap_data()
